...
public class AuthzConfigurationParserTest extends TestCase {
  ...
  public static final String DN_WITH_COLON = "CN=Robot:argo-egi@cro-ngi.hr, O=SRCE, O=Robots, C=HR, DC=EGI, DC=EU";
  ...
  protected void assertEqualPermissions(String message, PAPPermission p,
    PAPPermission p2) {

    assertTrue(message, p.satisfies(p2) && p2.satisfies(p2));
  }

  private void checkDnPermissions(String principal, String permString,
    ACL acl) {
  ...
  private void checkPermissions(PAPAdmin admin, PAPPermission perms, ACL acl) {
    ...
    assertNotNull("Permissions not found for admin '" + admin + "'.",
      acl.getPermissions()
        .get(admin));
    PAPPermission parsedPerms = acl.getPermissions()
      .get(admin);
    assertEqualPermissions("Parsed permissions do not match!", perms,
      parsedPerms);
  ...
  public void testEmptyFileParseSuccess() {
    ...
    assertTrue("The parsed ACL should be empty!", acl.getPermissions()
      .isEmpty());
  ...
  public void testOpensslFormatDN() {
    ...
    assertFalse("The parsed ACL should contain one entry!", acl.getPermissions()
      .isEmpty());
  ...
  public void testRoleFQANLongFormatParsing() {
    ...
    confFile.append(FQAN_STANZA);
    confFile.append(
      buildFQANPermLine(LEGAL_FQAN_WITH_ROLE_LONG_FORMAT, ALL_PERMISSIONS));
    ...
    checkFQANPermissions(LEGAL_FQAN_WITH_ROLE_LONG_FORMAT, ALL_PERMISSIONS,
      acl);
  ...
  public void testIllegalFQANParsing() {
    ...
    assertTrue("Configuration parser succeded in parsing an illegal fqan!",
      caughtException);
  }

  public void testDNwithColonAsPrincipalInACL() {

    AuthzConfigurationParser parser = AuthzConfigurationParser.instance();
    StringBuilder confFile = new StringBuilder();
    confFile.append(DN_STANZA);
    confFile.append(buildDNPermLine(DN_WITH_COLON, ALL_PERMISSIONS));

    ACL acl = parser.parse(new StringReader(confFile.toString()));
    assertNotNull(acl);
    checkDnPermissions(DN_WITH_COLON, ALL_PERMISSIONS, acl);
  }
