...
public class FileSystemPolicyDAO implements PolicyDAO {
	...
	public void delete(String papId, String policyId) {
		if (exists(papId, policyId)) {
			File policyFile = new File(RepositoryManager.getPolicyAbsolutePath(papId, policyId));
			policyFile.delete();
		}
	}

	public boolean exists(String papId, String policyId) {
		return new File(RepositoryManager.getPolicyAbsolutePath(papId, policyId)).exists();
	}
	...
	public void store(String papId, Policy policy) {
		if (!exists(papId, policy.getId())) {
			policy.printXACMLDOMToFile(RepositoryManager.getPolicyAbsolutePath(papId, policy.getId()));
		}
	}

	public void update(String papId, Policy policy) {
		File policyFile = new File(RepositoryManager.getPolicyAbsolutePath(papId, policy.getId()));
		if (!policyFile.exists()) {
