...
public class FileSystemPolicyDAO implements PolicyDAO {
	...
	policyFileNamePrefix = FileSystemRepositoryManager
	...
	if (exists(papId, policyId)) {
	    File policyFile = new File(FileSystemRepositoryManager
		    .getPolicyAbsolutePath(papId, policyId));
	    policyFile.delete();
	}
    }
	...
	return new File(FileSystemRepositoryManager.getPolicyAbsolutePath(
		papId, policyId)).exists();
    }
	...
	if (!exists(papId, policy.getPolicyId())) {
	    policyHelper
		    .toFile(FileSystemRepositoryManager.getPolicyAbsolutePath(
			    papId, policy.getPolicyId()), policy);
	}
    }

    public void update(String papId, PolicyType policy) {
	File policyFile = new File(FileSystemRepositoryManager
		.getPolicyAbsolutePath(papId, policy.getPolicyId()));
	if (!policyFile.exists()) {
	    throw new RepositoryException("PolicySet does not exist: "
		    + policy.getPolicyId());
	}
