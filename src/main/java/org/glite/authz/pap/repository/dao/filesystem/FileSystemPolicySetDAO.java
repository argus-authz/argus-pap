...
public class FileSystemPolicySetDAO implements PolicySetDAO {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(FileSystemPolicySetDAO.class);
	...
	policySetFileNamePrefix = FileSystemRepositoryManager
		.getPolicySetFileNamePrefix();
	policySetHelper = PolicySetHelper.getInstance();
    }

    public void delete(String papId, String policySetId) {
	File policySetFile = new File(FileSystemRepositoryManager
		.getPolicySetAbsolutePath(papId, policySetId));
	if (!policySetFile.delete()) {
	    throw new RepositoryException("Cannot delete policyset: "
		    + policySetId);
	}
    }

    public void deleteAll(String papId) {
	File papDir = new File(FileSystemRepositoryManager
		.getPAPDirAbsolutePath(papId));
	...
	return new File(FileSystemRepositoryManager.getPolicySetAbsolutePath(
		papId, policySetId)).exists();
    }
	...
	File policySetFile = new File(FileSystemRepositoryManager
		.getPolicySetAbsolutePath(papId, ps.getPolicySetId()));
	if (!policySetFile.exists()) {
	    throw new RepositoryException("PolicySet does not exist: "
		    + ps.getPolicySetId());
	}
