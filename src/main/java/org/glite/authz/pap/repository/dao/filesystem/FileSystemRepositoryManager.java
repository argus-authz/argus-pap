...
public class FileSystemRepositoryManager extends RepositoryManager {
    ...
    static {
	fileSystemDatabaseDir = PAPConfiguration.instance()
		.getPAPRepositoryDir();
	...
	File rootDir = new File(FileSystemRepositoryManager
		.getFileSystemDatabaseDir());
	if (!rootDir.exists()) {
	    if (!rootDir.mkdirs()) {
		throw new RepositoryException(
			"Cannot create DB root directory: "
				+ rootDir.getAbsolutePath());
	    }
	}
	if (!(rootDir.canRead() && rootDir.canWrite())) {
	    throw new RepositoryException("Permission denied for root DB dir: "
		    + rootDir.getAbsolutePath());
	}
