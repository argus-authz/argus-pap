...
public class PersistenceManager extends RepositoryManager {
    ...
    public void commitManagedTransaction(Transaction tx) {
        if (tx != null) {
            tx.commit();
        }
    }
...
//    }
    ...
    public void initialize() {
        ...
        configuration.setProperty(Environment.URL, hsqlDbUrl);
        ...
        configuration.setProperty(Environment.C3P0_TIMEOUT, "300");
        configuration.setProperty(Environment.C3P0_MAX_STATEMENTS, "50");
        configuration.setProperty(Environment.C3P0_IDLE_TEST_PERIOD, "3000");

        // annotated classes
        configuration.addAnnotatedClass(org.glite.authz.pap.common.Pap.class);
        configuration.addAnnotatedClass(org.glite.authz.pap.common.xacml.impl.PolicySetTypeString.class);
        configuration.addAnnotatedClass(org.glite.authz.pap.common.xacml.impl.PolicyTypeString.class);

        sessionFactory = configuration.buildSessionFactory();

        dbSchemaExport(configuration);
        
        initialized = true;
    }

    public Transaction manageTransaction() {
    ...
    public void rollbackManagedTransaction(Transaction tx) {
        if (tx != null) {
            tx.rollback();
        }
    }
    
    private void createTextTables() {

        Session session = openSession();
        
        session.getTransaction().begin();

        String repoDir = PAPConfiguration.instance().getPAPRepositoryDir() + File.separator;
        File tableFile = new File(repoDir + "Pap.csv");

        if (!tableFile.exists()) {

            session.createSQLQuery("CREATE TEXT TABLE PAP(ID VARCHAR(255) NOT NULL PRIMARY KEY,ALIAS VARCHAR(255),"
              ...
              .executeUpdate();
            session.createSQLQuery("SET TABLE PAP SOURCE \"Pap.csv\"").executeUpdate();
        }

        tableFile = new File(repoDir + "Policy.csv");

        if (!tableFile.exists()) {

            session.createSQLQuery("CREATE TEXT TABLE POLICY(ID VARCHAR(255) NOT NULL PRIMARY KEY,"
                    + "PAP_ID VARCHAR(255),POLICY_STRING VARCHAR(255))").executeUpdate();
            session.createSQLQuery("SET TABLE POLICY SOURCE \"Policy.csv\"").executeUpdate();
        }

        tableFile = new File(repoDir + "Policyset.csv");

        if (!tableFile.exists()) {
            session.createSQLQuery("CREATE TEXT TABLE POLICYSET(ID VARCHAR(255) NOT NULL PRIMARY KEY,"
                    + "PAP_ID VARCHAR(255),POLICYSET_STRING VARCHAR(255))").executeUpdate();
            session.createSQLQuery("SET TABLE POLICYSET SOURCE \"Policyset.csv\"").executeUpdate();
        }
     ...
     * @param ejb3Config
     */
    private void dbSchemaExport(AnnotationConfiguration annotatedConfiguration) {
