...
/**
 * This class provides methods to manage {@link Pap} objects (i.e. add, update, delete, etc). It also provides methods
 * to manage the <code>Pap</code> ordering. All of these methods affects both the repository and the configuration.
 * <p>
 * Use the {@link PapManager#initialize()} method before doing anything with this class.
 * <p>
 * The PAP service has always one (special) pap defined. This pap is commonly identified in the code with the word
 * <i>default</i>. The alias of this special pap is defined by the constant {@link Pap#DEFAULT_PAP_ALIAS} (its value is
 * {@value Pap#DEFAULT_PAP_ALIAS}). The <i>default</i> pap is local and public.
 * <p>
 * <b>Paps ordering</b><br>
 * For the policy evaluation process (performed by the <i>PDP</i>) the oder of the policies matters. Therefore it is
 * possible to specify an ordering for the <code>Pap</code> objects (which are containers of policies). The policies of
 * <code>Pap</code> <i>A</i> are evaluated before the policies of <code>Pap</code> <i>B</i> if <i>A</i> comes before
 * <i>B</i> in the order. There are two points concerning the pap ordering that must be kept in mind:<br>
 * 1. if the <i>defdault</i> pap is <b>not</b> listed in the pap ordering then it is automatically placed as the first
 * pap in the order;<br>
 * 2. if the pap ordering does not contains all the defined paps then the paps not listed in the order comes after
 * (except the <i>default</i> pap, see point 1).
 * <p>
 * <b>Repository and configuration</b><br>
 * The information associated to a <code>Pap</code> object are stored both in the repository and in the configuration.
 * Since the configuration can be changed off-line (when the PAP service is not running), synchronization between the
 * configuration and the repository is needed at startup time. See the method
 * {@link PapManager#synchronizeRepositoryWithConfiguration()}.<br>
...
public class PapManager {
    ...
    private static String[] configurationPapOrdering;
    private static DistributionConfiguration distributionConfiguration;
    ...
    public static PapManager getInstance() {
        ...
        return new PapManager();
    }

    /**
     * Initialization method. The main purpose of this method is to create the <i>default</i> pap if it doesn't exits
     * and to synchronize the paps defined in the configuration file with the ones stored in the repository.
     * 
     * @throws PapManagerException if this method has already been called.
     */
    public static void initialize() {
        if (isInitialized) {
            throw new PapManagerException("PapManager has been already initialized");
        }
        ...
        PapDAO papDAO = DAOFactory.getDAOFactory().getPapDAO();

        createDefaultPapIfNotExists(papDAO);
        synchronizeRepositoryWithConfiguration(papDAO);
    ...
    private static void createDefaultPapIfNotExists(PapDAO papDAO) {
        ...
        PapContainer defaultPapContainer = new PapContainer(papDAO.get(Pap.DEFAULT_PAP_ALIAS));
    ...
    /**
     * The information associated to a <code>Pap</code> object are stored both in the repository and in the
     * configuration. Since the configuration can be changed off-line (when the PAP service is not running),
     * synchronization between the configuration and the repository is needed. This method synchronize these information
     * doing the following steps:<br>
     * 1. add to the repository all the paps defined in the configuration that are not present in the repository;<br>
     * 2. if a pap is defined both in the repository and in the configuration then update the information in the
     * repository with the one found in the configuration (if they differs);<br>
     * 3. delete from the repository the paps that are not in the configuration.
     */
    private static void synchronizeRepositoryWithConfiguration(PapDAO papDAO) {
     ...
     * The paps in the list are ordered following the pap ordering as specified in the configuration.
     * 
     ...
     * If the given array is <code>null</code> or <code>empty</code> then the previous order (if any) is cleared.
     * 
     * @param aliasArray array of pap aliases defining the new pap ordering (can be <code>null</code> or
     *            <code>empty</code>).
    ...
    /**
     * Builds a list of alias of all the defined paps that follows the pap ordering defined in the configuration.
     * 
     * @return a list of alias of all the defined paps that follows the pap ordering defined in the configuration.
     */
