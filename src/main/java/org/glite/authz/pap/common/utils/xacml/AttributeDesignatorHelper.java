...
public class AttributeDesignatorHelper extends
	XACMLHelper<AttributeDesignatorType> {
    private static AttributeDesignatorHelper instance = null;

    public static AttributeDesignatorType build(QName designatorType,
	    AttributeType attribute) {
	AttributeDesignatorType attributeDesignator = (AttributeDesignatorType) Configuration
		.getBuilderFactory().getBuilder(designatorType).buildObject(
			designatorType);
