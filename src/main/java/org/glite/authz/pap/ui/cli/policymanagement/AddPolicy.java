...
public class AddPolicy extends PolicyManagementCLI {
    ...
    private static final String LONG_DESCRIPTION = "This command allows to add a rule into an action by specifying "
	    + "an action-id of an existing action (option --" + OPT_ACTIONID_LONG
	    + ") or a resource/action value (options --" + OPT_RESOURCE_LONG + " and --" + OPT_ACTION_LONG
	...
	if (actionId == null) {
		...
		throw new ParseException(String.format("Options --%s and --%s need option --%s.",
			OPT_BEFORE_RULEID_LONG, OPT_AFTER_RULEID_LONG, OPT_ACTIONID_LONG));
	    }
		...
		throw new ParseException(
			String.format("--%s and --%s must be both present.", OPT_RESOURCE_LONG, OPT_ACTION_LONG));
	    }
	    if (ruleId != null) {
		throw new ParseException(String.format(
			"Options --%s and --%s need option --%s and cannot be used with options --%s and --%s.",
			OPT_BEFORE_RULEID_LONG, OPT_AFTER_RULEID_LONG, OPT_ACTIONID_LONG, OPT_RESOURCE_LONG,
			OPT_ACTION_LONG));
	    }
	} else {
		...
		throw new ParseException(String.format("Option --%s cannot be used with options --%s and --%s.",
			OPT_ACTIONID_LONG, OPT_RESOURCE_LONG, OPT_ACTION_LONG));
	    }
	}
	...
	if (bottom && (ruleId != null)) {
	    throw new ParseException(
		    "Specify one of --" + OPT_BOTTOM_LONG + " option or \"rule-id\", not both at the same time.");
	}
	...
	if (commandLine.hasOption(OPT_OBLIGATION_LONG)) {
		...
		if (osParsedValue.trim().equals("action") || osParsedValue.trim().equals("resource")) {
		    ...
		    throw new ParseException("Unrecognized value for the --obligation-scope option: '" + osParsedValue
	...
	policyId = highlevelPolicyMgmtClient.addRule(alias, isPermit,
		attributeList.toArray(new String[attributeList.size()]), actionValue, resourceValue, actionId, ruleId,
		obligationId, obligationScope, afterRule);
