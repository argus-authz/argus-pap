...
public class ExceptionsRule {
	...
	public static RuleType build(List<AttributeType> sbjAttr,
			List<AttributeType> rsrcAttr, List<AttributeType> envAttr, EffectType effect) {
		...
		for (List<AttributeType> list:listOfLists) {
			applyOr.getExpressions().add(createAnyOfAll(list, AttributeDesignatorType.SUBJECT_ATTRIBUTE_DESIGNATOR_ELEMENT_NAME));
		}
		
		listOfLists = getSubLists(rsrcAttr);
		for (List<AttributeType> list:listOfLists) {
			applyOr.getExpressions().add(createAnyOfAll(list, AttributeDesignatorType.RESOURCE_ATTRIBUTE_DESIGNATOR_ELEMENT_NAME));
		}
		
		listOfLists = getSubLists(envAttr);
		for (List<AttributeType> list:listOfLists) {
			applyOr.getExpressions().add(createAnyOfAll(list, AttributeDesignatorType.ENVIRONMENT_ATTRIBUTE_DESIGNATOR_ELEMENT_NAME));
		}
		...
		if (!attributeList.isEmpty()) {
			for (AttributeType attribute : attributeList) {
				...
				for (XMLObject elem : attribute.getAttributeValues()) {
					applyStringBag.getExpressions().add(PolicyAttributeValueHelper.build(dataType,
															((AttributeValueType) elem).getValue()));
				}
			}
			applyAnyOfAll.getExpressions().add(applyStringBag);
			applyAnyOfAll.getExpressions().add(AttributeDesignatorHelper.build(designatorType, attributeList.get(0)));
		}
